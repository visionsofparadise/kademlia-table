...
const table = new KademliaTable(randomBytes(8));

const randomId = () => randomBytes(8);

it("returns the correct distance between ids", () => {
	const a = Buffer.from("0000", "hex");
	const b = Buffer.from("0001", "hex");

	const result = KademliaTable.getDistance(a, b);

	expect(result).toBe(1);
});

it("creates a compare function that correctly sorts ids by distance", () => {
	const targetId = Buffer.from("0000", "hex");

	const ids = ["4545", "a5a5", "1111"].map((id) => Buffer.from(id, "hex"));

	const compare = KademliaTable.createCompare(targetId);

	const result = ids.sort(compare);

	expect(result.map((buffer) => buffer.toString("hex"))).toStrictEqual(["1111", "4545", "a5a5"]);
});
...
it("returns false when node added but bucket full", () => {
	const customTable = new KademliaTable(Buffer.from("00000000", "hex"), { bucketSize: 10 });
...
it("gets correct i for id", () => {
	const customTable = new KademliaTable(Buffer.from("0000", "hex"));
...
it("gets 20 closest nodes out of 1000", () => {
	const customTable = new KademliaTable(randomId());
...
it("sends node to tail of bucket on seen", () => {
	const customTable = new KademliaTable(Buffer.from("00000000", "hex"));
